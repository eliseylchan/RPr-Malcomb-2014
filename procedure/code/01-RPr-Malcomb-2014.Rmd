...
author: "Joseph Holler, Kufre Udoh, Drew An-Pham, Elise Chan, Middlebury Open GIScience Classes"
date: "`r Sys.Date()`"
...
```{r clip-map-flood-exposure, warning = F}
names(fl) <- "fl"
...
### New Planned Deviation: Map Vulnerability by Traditional Authority

The original study does not compute the vulnerability score for each TA, which is the scale that the social data is aggregated on. Instead, a vulnerability score is calculated per pixel in the raster. To help visualize vulnerability scores with the goal of aiding policy makers distribute resources among administrative areas, I calculated the climate change vulnerability score for the TAs. 

The TA vulnerability score was calculated by taking the mean vulnerability score of the all pixels in the TA. Not all TAs had an adaptive capacity score, so not every TA received a score.

```{r compute-zonal-stats}
# select TAs and relevant info. 134 TAs.
zonal_ta_input <- ta_v %>% 
  select(geom, NAME_2, TYPE_2) %>% 
  filter(TYPE_2 == "Traditional Authority")

# take mean vulnerability score across TA
ta_vuln <- aggregate(vulnerability_r, zonal_ta_input, FUN = mean) %>% 
  st_as_sf()

# plot vulnerability score for each TA
ta_vuln_map <- ggplot() + 
  geom_sf(
    data = ta_vuln, 
    aes(fill = cut(vulnerability, breaks = 4), 
        label = vulnerability),
    color = "grey98"
  ) +
  #geom_sf_text(data = ta_vuln, aes(label = round(vulnerability, digits = 0)), size = 1) +
  scale_fill_manual(
    name = "Mean Vulnerability Score Per TA", 
    values = c("lemonchiffon1", "orange", "lightcoral", "firebrick"),
  ) +
  scale_y_continuous(breaks=seq(-17,-10,1)) +
  scale_x_continuous(breaks=seq(33,36,1)) +
...
#export plot
ggsave("ta_vuln_map.jpg", plot = ta_vuln_map)
```

To pull what score is associated with each TA by TA name, the TA polygons with the score need to be joined to the TA names.

The TA with the lowest vulnerability score is Kasukula (35.9).
The TA with the highest vulnerability score is Lundu (73.4).

```{r zonal-summary-statistic}
# join polygons to TA names from ta_v to identify TAs. Has 111 TAs with valid vulnerability scores.
ta_vuln_with_names <- st_join(ta_vuln, ta_v, join = st_equals, left = TRUE) %>% 
...
# TA with min score
ta_vuln_with_names %>% 
  filter(vulnerability == min(vulnerability)) %>% 
  pull(NAME_2, vulnerability)

# TA with max score
ta_vuln_with_names %>% 
  filter(vulnerability == max(vulnerability)) %>% 
  pull(NAME_2, vulnerability)
```
...
The reproduction tends to assign a lower Adaptive Capacity score to each TA than the original study (Figure 4).
The reproduction was only higher than the original 4 times, and all were located in the south of Malawi.
The reproduction assigned the same score 82 times, and underestimated the score 111 times.
The Spearman's Rho is 0.789, suggesting that the reproduction consistently made predictions when compared with the original scores.
I wonder if this comes from the interpretation of the quintile scale as being from 0 to 4 instead of 1-5 (i.e. a larger bin count) which changed the normalization of the data.

The map comparing the original and reproduction vulnerability risk assessment (Figure 5) shows that the reproduction tended to assign lower vulnerability scores than the original study.
However, the Spearman Rho coefficient of 0.197 suggests the values assigned have very little correlation to those of the original study.
The vulnerability score is based on the adaptive capacity score, so errors in the adaptive capacity could propagate into the uncertainty we see in the vulnerability reproduced score.
However, the vulnerability score is an *additive* aggregation, so there must have been errors in the other factors in order to yield the random predictions.
The added map of vulnerability scores at the TA level shows the areas of highest vulnerability clustered in the southern-most and northern-most parts of Malawi, as well as on the southern shore of Lake Malawi. Calculating vulnerability score by an administrative area like TAs would aid policy makers determine how to distribute resources in Malawi, which was one of the study's goals.

## Discrepancies in Original Study
...
A modification to the original study is calculating climate change vulnerability scores for each traditional authority.

# Conclusions

We were able to reproduce maps of climate vulnerability score across Malawi. However, our results did not perfectly align with the results of the original study and underestimated the original study's score for over half of the traditional authorities. This can be attributed to discrepancies in the traditional authority data set. Calculating vulnerability scores by TA also revealed the method's sensitivity to partition distortion.
